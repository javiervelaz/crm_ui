...
interface Props {
  data: Fijo[];
  onUpdate: (data: Fijo[]) => void;
  categorias: any[];
}

export default function PlanillaFijos({ data = [], onUpdate, categorias = [] }: Props) {
  const [items, setItems] = useState<Fijo[]>(data || []);
  const memoizedOnUpdate = useCallback(onUpdate, [onUpdate]);

  const addRow = () => {
    const newItems = [...items, { servicio: "", monto: 0 }];
    setItems(newItems);
    onUpdate(newItems);
  };

  const updateRow = (index: number, field: keyof Fijo, value: any) => {
    const updated = [...items];
  ...
  return (
    <div>
      <h2 className="mb-4 text-lg font-bold text-brand-800">Gastos Fijos</h2>
        ...
        {items.map((row, i) => (
          <div key={i} className="rounded-xl border border-brand-100 bg-brand-50 p-3 space-y-2">
            <div className="flex items-center justify-between gap-2">
              <select
                value={row.servicio}
                onChange={(e) => updateRow(i, "servicio", e.target.value)}
        ...
        <table className="w-full border-collapse rounded-lg overflow-hidden text-sm">
          <thead>
            <tr className="bg-brand-50">
              <th className="px-4 py-2 text-left text-xs font-semibold text-brand-600 uppercase tracking-wider border-b border-brand-100">Servicio</th>
