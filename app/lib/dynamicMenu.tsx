...
export default function DynamicMenu() {
  ...
  if (loading) {
    ...
    return (
      <aside className="w-64 bg-brand-800 min-h-screen">
        <div className="p-4 animate-pulse">
          <div className="h-8 bg-brand-700 rounded mb-4"></div>
          <div className="h-6 bg-brand-700 rounded mb-2"></div>
          <div className="h-6 bg-brand-700 rounded mb-2"></div>
          <div className="h-6 bg-brand-700 rounded"></div>
        </div>
      </aside>
    );
  }
  ...
  return (
    <nav className="flex flex-col flex-1 overflow-y-auto">
      ...
      <div className="mt-auto p-4 border-t border-brand-700">
        <button
          onClick={handleLogout}
          className="flex items-center gap-2 text-white/50 hover:text-accent-600 transition-colors"
        >
          <LogOut size={18} />
          Cerrar sesión
        </button>
